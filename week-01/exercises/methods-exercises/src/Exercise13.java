public class Exercise13 {
    ...
    // (See Exercise07.)
    private static boolean areInOrder(int low, int mid, int high) {
    ...
    // 7, 5, 6 -> false
    // 1, 0, 1 -> true
    private static boolean areContiguous(int first, int second, int third) {
        return Math.abs(first - second) == 1 && Math.abs(second - third) == 1;
    }
    // 3. Create a method.
    ...
    // the problem.
    private static boolean isAscendingContiguous(int low, int med, int high) {
        return areInOrder(low, med, high) && areContiguous(low, med, high);
    }
